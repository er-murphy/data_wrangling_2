...
For exact matches, you can designate matches at the beginning or end of a line. The first example here searches for a match at the beginning of the line, while the second example searches for a match at the end of the line.
...
You don’t have to list these; instead, you can provide a range of letters or numbers that count as a match. For example, in this case we're searching for strings starting with a number 0 through 9, followed by an upper or lowercase letter.
...
# Thoughts on Factors
Factors are maybe the least intuitive of `R`’s data types. They are very useful, but they also do things you don’t expect; this is especially bad when you have factors but think you have characters – which happens more than you’d expect, because R uses factors a lot (mostly for historical reasons).
...
For this reason, it’s important to be deliberate with factors, particularly by using functions in `forcats`.
...
Let’s quickly visualize these data for the 12-17 age group; to make the plot readable, we’ll treat `State` as a factor and reorder according to the median `percent` value.
...
The ordering of factor variables play an important role in linear regression. Specifically, the ordering determines the “reference” category, and is something that can be adjusted as needed.
